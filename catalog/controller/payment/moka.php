...
namespace Opencart\Catalog\Controller\Extension\Moka\Payment;

use Opencart\System\Engine\Controller;

require_once(DIR_EXTENSION . '/moka/moka-php/autoload.php');
...
class Moka extends Controller
{
    ...
    public function index()
    {
        ...
        $data['checkout'] = html_entity_decode($this->url->link('extension/moka/payment/moka.checkout', '', true), ENT_COMPAT, 'UTF-8');
        $data['installment'] = html_entity_decode($this->url->link('extension/moka/payment/moka.installment', '', true), ENT_COMPAT, 'UTF-8');

        return $this->load->view('extension/moka/payment/moka', $data);
    }
    ...
    public function checkout()
    {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            ...
            $callback_url = html_entity_decode($this->url->link('extension/moka/payment/moka.callback', '', true), ENT_COMPAT, "UTF-8");
    ...
    public function installment()
    {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateInstallment()) {
            ...
            $data['html'] = $this->load->view('extension/moka/payment/moka_installment', $installmentDetails);
        ...
        if (isset($this->request->post['hashValue'])) {
            ...
            if ($code_for_hash == $hashValue) {
                ...
                if ($paymentDetail_result_code == 'Success') {
                    $this->model_extension_moka_payment_moka->addTransaciton($paymentDetail_data->PaymentDetail, $order_id);
                }

                $this->model_checkout_order->addHistory($order_id, $this->config->get('payment_moka_order_status'));

                $data['redirect'] = $this->url->link('checkout/success', 'language=' . $this->config->get('config_language'), true);

                return $this->response->setOutput($this->load->view('extension/moka/payment/moka_success', $data));
            } else {
                $data['redirect'] = $this->url->link('checkout/failure', 'language=' . $this->config->get('config_language'), true);

                return $this->response->setOutput($this->load->view('extension/moka/payment/moka_failure', $data));
            }
        }
    }
    ...
    private function validate()
    {
        ...
        if (!isset($this->request->post['card_holder_full_name']) || empty(trim($this->request->post['card_holder_full_name']))) {
    ...
    private function validateInstallment()
    {
        ...
        if (!isset($this->request->post['bin_number']) || $this->request->post['bin_number'] < 6) {
